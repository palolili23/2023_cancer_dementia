...
data_wide %<>% mutate(
  ...
  sw_death = if_else(competing_plr == 0, 1/(1-p_death_den), 1))
...
results_output_unstabilized <- list(cancer_ever_km_crude, cancer_ever_cox_crude,
                       cancer_ever_km, cancer_ever_cox, 
                       km_t2cancer, cox_t2cancer, km_t2cancer_iptw, cox_t2cancer_iptw,
                       km_t2cancer_ipcw, cox_t2cancer_ipcw)

saveRDS(results_output_unstabilized, here::here("02_R", "unstab_results.rds"))
# 3.d. Lower bound ------------------------------------------------------------------
