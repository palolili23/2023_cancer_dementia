...
A major challenge related to assigning cancer diagnosis as the proxy of Pin1 is defining time zero. Not everyone who had Pin1 overexpression will be diagnosed with cancer by age 65. For this reason prospective cohort studies have considered cancer as a time-varying or time-dependent exposure. This means that a participant who had cancer diagnosis at age 70 contributes to the "regulated Pin1" from age 65 to 70, and from age 70 and onwards, this participant contributes to "Over-expressed Pin1" arm. Alternatively, studies that have included participants with cancer diagnosis at the time of the diagnosis (for example from cancer registries such as SEER) and matched participants by age. These subtleties are relevant for the next point. 

Since the population is elderly, the risk of death at age 65 is not trivial and it increases over time. This means that some individuals may die prior to cancer diagnosis due to other causes, such as cardiovascular disease which is one of the leading causes of death in this age group. As such, we can only measure $P^*$ in the subset of individuals who have survived long enough to have a cancer diagnosis. We zoom in the relationship of $P \rightarrow P^*$ in Figure 3. In this figure we include 
...
Death is a competing event of ADRD because if a participant dies prior to ADRD diagnosis, death prevents from observing ADRD at future time-points. The causal diagram in Figure 4 is an extension of Figure 2, including $D_{80}$ as a representation of Death by age 80. We include an arrow between $P$ and $D_{80}$ since $P$ may increase the risk, through tumor development or other shared common causes. Furthermore we include an arrow between $P^*$ and $D_{80}$ since cancer diagnosis, and subsequent treatment (or lack of treatment) may have an effect on death. The arrow from $D_{80}$ and $Y_{85}$ represents the key feature of a competing events data structure: an individual who dies by age 80 cannot subsequently develop ADRD. Last, since $D_{80}$ and $Y_{85}$ events that are related to the aging process, $C$ represent the shared causes related to both events (sch as cardiovascular conditions, fraility, other comorbidities, etc).

If we had measured all $L$, we could estimate the total effect of $P$ in the risk of ADRD under no additional assumptions. This effect includes all the pathways including those mediaded through death.

There are several causal estimands that can help us identify the direct of $P$ in $Y_{85}$ as presented by Young et al. 
However if we want to isolate the direct effect of P in Y (path in violet), we
