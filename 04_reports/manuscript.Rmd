---
title: "Towards a clearer causal question underlying the association between cancer and ADRD"
author: ""
date: ""
...
**Figure 1. Pin1 as biomarker and risk of dementia.** Causal directed acyclic graph where Pin1 at time $t$ is represented as $P_t$, Dementia at time $t+k$ is represented as $Y_{t+k}$. $L_1$ represents shared causes of Pin1 and Dementia, such as smoking. To isolate the effect of $P_t$ and $Y_{y+k}$ we need to block the backdoor path $Y_{t+k} \leftarrow L_1 \rightarrow P_t$. 

```{tikz, echo = FALSE, fig.cap = " Incident cancer diagnosis as proxy for Pin1 expression"}
\usetikzlibrary{arrows}
\begin{tikzpicture}[node distance=2cm]
\node (p) {$P_t$};
\node [above of = p, yshift = -0.8cm, xshift = -1cm](l) {$L_1$};
...
**Figure 2. Incident cancer diagnosis as proxy for Pin1 expression.** This causal graph is an expansion of Figure 1, includes Pin1 expression at time $P_{t+1}$. Incident cancer diagnosis is represented as $C_t$ and $C_{t+1}$. To isolate the effect of $P_t$ and $P_{t+1}$ on $Y_{y+k}$ we need to block the backdoor path $Y_{t+k} \leftarrow L_1 \rightarrow P_t$ and $Y_{t+k} \leftarrow L_1 \rightarrow P_{t+1}$. Although we represent $L_1$ as a single node for readability, $L_1$ is time-varying too.
...
**Figure 3. Time-varying cancer diagnosis and inmortal-time bias.**  This causal graph is an expansion of Figure 2, where $D_{t+1}$ represents death at time $t+1$. Cancer diagnosis at $t+1$ can only be measured among those who are alive at $t+1$. 
...
**Figure 4. Death as competing event of dementia.** In this graph we only focus attention to the exposure as if it was measured for all at time $P_{t+1}$. We include $D_{t+k}$ as death at time $t+k$, $L_2$ represents possible shared causes of dementia and death (such as cardiovascular comorbidities). Pin1 may affect the risk of death through cancer diagnosis, represented as an arrow between $C_{t+1}$ and  $D_{t+k}$. To isolate the direct effect of $P_{t+1}$ and $Y_{y+k}$, we have to block the backdoor pathway from $Y_{t+k} \leftarrow L_2 \rightarrow D_{t+k} \leftarrow C_{t+1} \rightarrow P_{t+1}$ and $Y_{t+k} \rightarrow D_{t+k} \leftarrow L_1 \rightarrow P_{t+1}$
...
**Figure 5. Causal graph for effect of Pin1 through the proxy of cancer diagnosis in the risk of dementia, including all challenges discussed through Figures 1 to 4.**

```{r}
knitr::include_graphics(here::here("03_figs", "cif_plot.tiff"))
```
...
### C. Causes of death for participants who died prior to dementia diagnosis
...
Causes of death were defined by ICD10 codes...xxx