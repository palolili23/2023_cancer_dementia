...
## 4. Motivating example with the RS data
...
First we illustrate the most simple scenario (as Figure 1), were Pin1 over-expression is defined as "cancer ever vs. never" as if Pin1 and cancer diagnosis happened at the same time if looked retrospectively. Second, we consider cancer as a time-varying exposure independent of death. This can be represented as follows, at five years from study entry the observed unadjusted risk of cancer was `r cancer_5y` and the risk of death was `r death_5y` for the entire cohort, thus we assume that those who died would have the same risk of cancer had they not died, unconditionally, over follow-up. Third, we consider time to cancer as the  

To address confounding we fit inverse probability treatment weights including age at study entry, sex, educational attainment, smoking status and cohort as covariates. 

To identify the controlled direct effect we considered death as an independent censoring mechanism conditional on time-varying shared causes of death and dementia. 
