...
\node [below of = d, yshift = 1cm, xshift = 1cm, gray!30](cd) {$C$};
\draw[->, gray!30] (l) to (dp);
\draw[->, gray!30] (l) to [out=0,in=90, looseness=0.4] (d);
\draw[->] (l) -- (p);
...
\draw[->, gray!30] (cd) -- (d);
\draw[->, gray!30] (cd) -- (y);
\draw[->, semithick] (p) to [out=45,in=135, looseness=0.2] (y);
\end{tikzpicture}
```
...
\begin{tikzpicture}[node distance=2cm]
\node (p) {$P_0$};
\node [above of = p, yshift = -0.8cm, xshift = -1cm, gray!30](l) {$L$};
\node [right of = p, gray!30] (dp) {$D_t$};
...
\node [right of = d] (y) {$Y_{t+1}$};
\node [below of = d, yshift = 1cm, xshift = 1cm,](cd) {$C$};
\draw[->, gray!30] (l) -- (p);
\draw[->, gray!30] (l) to [out=0,in=90, looseness=0.4] (y);
\draw[->, gray!30] (l) to (dp);
\draw[->, gray!30] (l) to [out=0,in=90, looseness=0.4] (d);
...
\draw[->, gray!30] (l) to [out=0,in=90, looseness=0.4] (y);
\draw[->, gray!30] (l) to (dp);
\draw[->] (l) to [out=0,in=90, looseness=0.4] (d);
\draw[->, gray!30] (dp) -- (pstar);
\draw[->] (p) to [out=45,in=135, looseness=0.3] (pstar);
\draw[->] (pstar) -- (d);
\draw[->] (d) -- (y);
